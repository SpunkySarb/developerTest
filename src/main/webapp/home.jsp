<!DOCTYPE html>
<html lang="en" dir="ltr">
  <head>
    <meta charset="utf-8">
    <title>DEV TEST</title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
     <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
<link rel="stylesheet" href="https://www.w3schools.com/w3css/4/w3.css">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
<link rel="stylesheet" href="style.css">
  </head>
  <body>

<div class="w3-bar w3-black w3-jumbo">
  <div class="w3-bar-item w3-left fa fa-bars">

  </div>
  <div class="w3-bar-item w3-rest w3-center  ">
<h3>Address Book</h3>
</div>
</div>

<br><br><br>

<div class="w3-content  w3-margin w3-round-large w3-card-4">
<div class="w3-container  w3-center w3-round w3-black">
<p>Login</p>
</div>
<br>
<div class="w3-center">
  <form class="" action="/login" method="post">
    <input type="text" name="username" class="w3-input  w3-round-large" value="" placeholder="Enter Username..."><br>
    <input type="text" name="username" class="w3-input  w3-round-large" value="" placeholder="Enter Password..."><br>
    <input type="submit" class="w3-button w3-black w3-round-large login" name="" value="Login"><br><br>

  </form>
  <div class="w3-button w3-center w3-black  w3-round-large create">
Create an Account
  </div>
  <br><br><br>
</div>



</div>



























<script src="javaScript.js" charset="utf-8"></script>
  </body>
</html>
